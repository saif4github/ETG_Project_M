...
static inline void MmwDemo_rxChanPhaseBiasCompensation(uint32_t *rxChComp,
                                                       int64_t *input,
                                                       uint32_t numAnt);
void DssPutAlert(uint32_t u32Alert);
uint32_t DssGetAlert(void);

extern volatile cycleLog_t gCycleLog;

uint8_t bAlertWarnSystem = 0;
bool velWarning = FALSE;
extern volatile uint8_t rangeRadarflag;

/* Adding defines for Tracks and Processing DDSA -RP*/
...
//float angleOfArrival[20];
float trackerOpFinalTTC[20];            //
uint8_t ObjTrackerFinialAge[20];
uint8_t trackerOpFinalTTC_iKIndex[20];
float TempTccThres[20];
volatile uint8_t WIDE_VIEW = 0;
void populateOutputs(MmwDemo_DSS_DataPathObj *obj)
{
    uint32_t ik,jk;

    // static uint32_t u32Alert =0;
    ...
    if (obj->processingPath == MAX_VEL_ENH_PROCESSING)
    {
        ...
        jk = 0; 
        const float ConstTtcCurveParam = 21.0f;
        const float ConstTTCUpperThres = 6.0f;
        const float ConstTTCLowerThres = 4.5f;

        const float constVelocity = 4.4704f; // Original 4.4704f
        const float constVelOffset = 0.2237f;
        const float ConstYThres = 12.00f; // for testing 2.00, for final production 12.00
        float XThres = 0.0f;
        static float ConstXThres;
        static float secondaryZone;
        static float primaryZone;
        if(rangeRadarflag == 1)
        {
            WIDE_VIEW = 1;
        }
        ...
        {
            ConstXThres = 1.5240f;
            secondaryZone =  0.1524f; // Wide View
            primaryZone = 0.1524f;
        }
        else
        {
            ConstXThres = 0.2286f;
            secondaryZone =  0.0254f; // Narrow View
            primaryZone =  0.0254f;
        }
        MmwDemo_dssAssert(obj->trackerInstance.maxTrackers == obj->dbScanInstance.maxClusters);
        memset(trackerOpFinalTTC,0,sizeof(trackerOpFinalTTC));
        //memset(angleOfArrival, 0, sizeof(angleOfArrival)); // clearing out previous frames AOA
        memset(trackerOpFinalTTC_iKIndex,0,sizeof(trackerOpFinalTTC_iKIndex));
        ...
        for (ik = 0; ik <  obj->trackerInstance.maxTrackers; ik ++)
        {
            ...
            // Remove going away targets
            if(obj->trackerState[ik].vec[iYd] > 0)
            {
                continue;
            }
            // vv
            ...
            trackerOpFinalTTC[jk]       =    obj->trackerState[ik].vec[iY]/obj->trackerState[ik].vec[iYd];  //distance / speed ==> gives time
            //angleOfArrival[jk] = ((atan(obj->trackerState[ik].vec[iY]/obj->trackerState[ik].vec[iX]))*57.2958);
            //trackerOpFinalTTC[jk]       =    (float)trackerOpFinal[ik].y/(float)trackerOpFinal[ik].yd;
            // trackerOpFinalTTC[jk]       =    (u16ConstTTCUpperThres -pow(((abs((float)(trackerOpFinal[ik].yd/oneQFormat))-1)/u16ConstTtcCurveParam),2.5));
            ObjTrackerFinialAge[jk] = obj->trackerState[ik].age;
            trackerOpFinalTTC_iKIndex[jk] = ik;
            jk ++;
        ...
        for (ik = 0; ik <  jk; ik ++)
        {
            ...
            float tempTTCThres = 0;

            /*Eliminate all the cases of secondary zone*/
            if (((float)abs(obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iX]) - ConstXThres) > secondaryZone)     /* 1 inch precision, 1 inch = 0.0254 m */
            {
                continue;
            }
            if (((float)obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iYd]) > -1.0f)
            {
                //nothing
                tempTTCThres = ConstTTCUpperThres;
            }
            else
            {
                // ????
                tempTTCThres  = (ConstTTCUpperThres -pow(((abs((float)(obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iYd]))-1)/ConstTtcCurveParam),2.5f));
            }

            TempTccThres[ik] = tempTTCThres;
            if(tempTTCThres > ConstTTCUpperThres)
            {
                tempTTCThres = ConstTTCUpperThres;
            }
            else if (tempTTCThres < ConstTTCLowerThres)
            {
                tempTTCThres = ConstTTCLowerThres;
            }
            else
            {
                /* Do Nothing*/
            ...
            if((float)(obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iX]) > 0.0f)    /* object in 1st Quadrant */
            {
                XThres = obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iX];
            }
            else if((float)(obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iX]) < 0.0f)   /* object in 2nd Quadrant */
            {
                XThres= (float)(obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iX]); /* add offset to be in safer side */
            }
            /* Vehicle is in Primary Zone, and Meeting TTC Creteria and Vehicle Speed is more than 10MPH. */
            if((trackerOpFinalTTC[ik] > (tempTTCThres * -1))    /* TTC check*/ //4.3
                    && (((float)abs(obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iYd]) - (constVelocity)) > -constVelOffset)     /* speed should be greater than 4.5 m/sec == ~10 mph, precision of 0.022mph */
                    && (((float)abs(XThres) - ConstXThres) < primaryZone)      /* Check within, Primary Zone, precision of 1 inch */
                    && ((float)abs(obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iY]) > ConstYThres))    /* Continues siren condition... how many meters after ttc condition satisfied, the siren should alarm */
            {
                bAlertWarnSystem  = 1;      // Alarm Siren
            }
            /* Vehicle is in Primary Zone and Vehicle Speed is more than 10MPH. */
            else if((trackerOpFinalTTC[ik] < (tempTTCThres * -1))    /* TTC check*/ //4.3
                    && (((float)abs(obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iYd]) - (constVelocity)) > -constVelOffset)     /* speed should be greater than 4.5 m/sec == ~10 mph, precision of 0.022mph */
                    && (((float)abs(XThres) - ConstXThres) < primaryZone)      /* Check within, Primary Zone, precision of 1 inch */
                    && ((float)abs(obj->trackerState[trackerOpFinalTTC_iKIndex[ik]].vec[iY]) > ConstYThres))    /* Continues siren condition... how many meters after ttc condition satisfied, the siren should alarm */
            {
                bAlertWarnSystem  = 2;      // 500ms Visual Alert
            }
        }
    }
...
void DssPutAlert(uint32_t u32Alert)
{
    if(u32Alert !=0)
    {
        bAlertWarnSystem = 0x01;
    }
    else
    {
        bAlertWarnSystem = 0x00;
    }
}

uint32_t DssGetAlert(void)
{
    return bAlertWarnSystem;
}
#endif
/**
 *  @b Description
